package org.nasdanika.models.terraform.ecore;
...
@EPackageNodeProcessorFactory(nsURI = TerraformPackage.eNS_URI)
public class EcoreGenTerraformProcessorsFactory {

	private Context context;
	
	public EcoreGenTerraformProcessorsFactory(Context context) {
		this.context = context;
	}
		
	@EPackageNodeProcessorFactory(
			label = "Terraform Model",
			description = "Model of Terraform configuration",
			actionPrototype = """
                    app-action:
                      content:
                        content-markdown:
		...
		return new EPackageNodeProcessor(config, context, prototypeProvider) {
			...
			protected void generateDiagramAndGraphActions(Collection<Label> labels, ProgressMonitor progressMonitor) {
				...
				try {
					...
					Files.writeString(new File("target/terraform.drawio").toPath(), document.save(null));
				} catch (Exception e) {
					e.printStackTrace();
